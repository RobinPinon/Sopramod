...
public class OverlayWebsocket extends WebSocketServer {
    ...
    private void Request(String request, List<OverlayVoteOption> voteOptions) {
        ...
        OverlayMessage msg=new OverlayMessage(request,totalVotes, Sopramod.getInstance().settings.votingMode == SopramodSettings.VotingMode.MAJORITY ? "MAJORITY" : "PERCENTAGE",options);
        String broadcastString = gson.toJson(msg);
        Sopramod.LOGGER.debug("Broadcasting: "+broadcastString);
