...
public class CreeperEvent extends AbstractInstantEvent {
    ...
    @Override
    public void init() {
        Sopramod.getInstance().eventHandler.getActivePlayers().forEach(serverPlayerEntity -> EntityType.CREEPER.spawn(serverPlayerEntity.level(), serverPlayerEntity.blockPosition().north(), EntitySpawnReason.EVENT));
    }
