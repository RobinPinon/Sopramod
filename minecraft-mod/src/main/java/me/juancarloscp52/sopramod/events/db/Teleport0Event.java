...
public class Teleport0Event extends AbstractInstantEvent {
    ...
    @Override
    public void init() {
        server = Sopramod.getInstance().eventHandler.server;
        Sopramod.getInstance().eventHandler.getActivePlayers().forEach(serverPlayerEntity -> {
            ...
            SopramodUtils.teleportPlayer(serverPlayerEntity, overworld, overworld.getRespawnData().pos().getCenter());
            SopramodUtils.clearPlayerArea(serverPlayerEntity);
        });
