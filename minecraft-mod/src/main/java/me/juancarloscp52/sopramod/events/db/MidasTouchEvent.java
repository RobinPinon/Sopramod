...
public class MidasTouchEvent extends AbstractTimedEvent {
    ...
    public void tick() {
        for (var player : Sopramod.getInstance().eventHandler.getActivePlayers()) {
            ...
            var mobs = world.getEntities(player, box, x -> x instanceof LivingEntity && x.isAlive() && !x.getType().is(SopramodTags.EntityTypeTags.IGNORED_BY_MIDAS_TOUCH));
            for (var mob : mobs) {
            ...
            for (EquipmentSlot slot : EquipmentSlot.values()) {
                ...
                if (itemStack.isEmpty() || itemStack.is(SopramodTags.ItemTags.IGNORED_BY_MIDAS_TOUCH) || itemStack.is(SopramodTags.ItemTags.MIDAS_TOUCH_GOLDEN_ITEMS))
                    continue;
