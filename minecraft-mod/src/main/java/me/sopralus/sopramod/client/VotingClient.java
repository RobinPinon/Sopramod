...
import com.poc.sopramod.client.websocket.OverlayServer;
import com.poc.sopramod.networking.ServerboundVotes;
...
public class VotingClient {
    ...
    public void newPoll(int voteID, List<Component> events) {
        ...
        if (this.size - 1 == events.size()) {
            this.voteID = voteID;
            this.events = events;
            this.events.add(Component.translatable("sopramod.voting.randomEvent"));
            totalVotesCount = 0;
            votes = new int[4];
            ...
            for (Component key : events){
                int width = client.font.width(Component.translatableEscape("sopramod.votes.display", 5, key));
                if(width > max)
                    max = width;
            }
            pollWidth = max;
    ...
    public void render(GuiGraphics drawContext) {
        if(SopramodClient.getInstance().integrationsSettings.showUpcomingEvents) {
            drawContext.drawString(client.font, Component.translatable("sopramod.voting.total", this.totalVotesCount), 10, 20, ARGB.color(255,255, 255, 255));
    ...
    public void renderPollElement(GuiGraphics drawContext, int i) {
        ...
        if(settings.showCurrentPercentage){
            Component percentage = Component.translatableEscape("sopramod.votes.percentage", Mth.floor(ratio * 100));
    ...
    public void sendPoll(int voteID, List<Component> events) {
        if (SopramodClient.getInstance().integrationsSettings.sendChatMessages)
