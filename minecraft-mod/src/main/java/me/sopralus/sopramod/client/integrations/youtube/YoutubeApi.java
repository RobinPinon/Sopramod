package com.poc.sopramod.client.integrations.youtube;
            ...
            _youtubeServer.createContext("/", new HttpHandler() {
                ...
                public void handle(HttpExchange req) throws IOException {
                    try {
                            ...
                            if (entity != null) {
                                try (var inStream = entity.getContent()) {
                                    ...
                                    if (accessToken != null && refreshToken != null) {
                                        var integrationsSettings = SopramodClient.getInstance().integrationsSettings;
                        ...
                        var res = "<html><body>"
                                + I18n.get(isSuccessful ? "sopramod.options.integrations.youtube.returnToGame"
                                        : "sopramod.options.integrations.youtube.error.auth")
                                + "</body></html>";
                        ...
                        callback.accept(isSuccessful, isSuccessful ? null
                                : Component.translatable("sopramod.options.integrations.youtube.error.auth"));
                    } catch (IOException ex) {
                        LOGGER.error(ex);

                        callback.accept(false, Component.translatable("sopramod.options.integrations.youtube.error.auth"));
                    } finally {
                        stopHttpServer();
                    }
                }
