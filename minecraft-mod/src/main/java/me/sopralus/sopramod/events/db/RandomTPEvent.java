...
public class RandomTPEvent extends AbstractInstantEvent {
    ...
    public void init() {
        Sopramod.getInstance().eventHandler.getActivePlayers().forEach(player -> {
            ...
            player.stopRiding();
            Sopramod.getInstance().eventHandler.server.getCommands().performPrefixedCommand(commandSourceStack, "spreadplayers " + player.position().x() + " " + player.position().z() + " 0 " + distance.value() + netherSafety + " false " + player.getName().getString());
    ...
    public Component getDescription() {
        return Component.translatable("events.sopramod.random_tp." + distance.name());
    }
