...
public class AdventureEvent extends AbstractTimedEvent {
    ...
    @Override
    public void init() {
        Sopramod.getInstance().eventHandler.getActivePlayers().forEach(serverPlayerEntity -> serverPlayerEntity.setGameMode(GameType.ADVENTURE));
    }

    @Override
    public void end() {
        Sopramod.getInstance().eventHandler.getActivePlayers().forEach(serverPlayerEntity -> serverPlayerEntity.setGameMode(GameType.SURVIVAL));
        super.end();
    }
